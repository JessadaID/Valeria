  ...
  async function insertOrder() {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.message || "เกิดข้อผิดพลาดในการเพิ่มรายการสั่งซื้อ",
        );
      } else {
    ...
    if (StoreUser.isLoggedIn()) {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          errorAlert(
            errorData.message || "เกิดข้อผิดพลาดในการสร้างการชำระเงิน",
          );
          throw new Error(
            errorData.message || "เกิดข้อผิดพลาดในการสร้างเซสชันชำระเงิน",
          );
        } else {
          await insertOrder();
                  ...
                  <div class="flex items-start space-x-4">
                    ...
                    <div class="flex-grow min-w-0">
                      ...
                      <div class="space-y-1 text-sm text-gray-500">
                        <p>
                          ประเภท: <span class="font-medium">{item.type}</span>
                        </p>
                        <p>
                          ลิขสิทธิ์:
                          <span
                        ...
                        <p>
                          AI Training:
                          <span
                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {!item.noAiTraining
                        ...
                        {#if item.imageSize}
                          <p>
                            ขนาด: {item.imageHeight} x {item.imageWidth} | {formatFileSize(
                              item.imageSize,
                            )}
                          </p>
                        {/if}
        ...
        {:else}
          ...
          >
            <svg
              ...
              stroke-linecap="round"
              stroke-linejoin="round"
              class="icon icon-tabler icons-tabler-outline icon-tabler-shopping-cart-plus mx-auto h-16 w-16 text-gray-300 mb-4"
            ...
            <p class="text-gray-600 mb-6">
              ยังไม่มีสินค้าในตะกร้าของคุณ เริ่มช็อปปิ้งเลย!
            </p>
            <a
              href="/"
      ...
      <div class="lg:col-span-1">
        ...
        >
          ...
          <div class="p-6">
            <div class="space-y-3 mb-6">
              <div class="flex justify-between text-sm">
                <span class="text-gray-600"
                  >ยอดรวมสินค้า ({itemcart.length} ไฟล์)</span
                >
            ...
            <div
              class="mt-4 flex items-center justify-center text-xs text-gray-500"
            >
              <svg
                class="w-4 h-4 mr-1"
                fill="none"
        ...
        <div class="mt-6 bg-green-50 rounded-xl p-4">
          <div class="flex items-start">
            ...
            <div>
              ...
              <p class="text-xs text-green-700">
                ไฟล์ความละเอียดสูง รองรับการใช้งานเชิงพาณิชย์
              </p>
              <p class="text-xs text-green-700">
                สามารถดาวน์โหลดซ้ำได้ใน 30 วัน
              </p>
            </div>
          </div>
        </div>
