                        ...
                        if (order.order_items && order.order_items.length > 0) {
                            const processedItems = await Promise.all(
                                order.order_items.map(async (item) => {
                                    if (item.image_id) {
                                        try {
                                            const pixabayApiKey = import.meta
...
<div class="min-h-screen bg-gray-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            >
                <div>
                    ...
                    <p class="text-gray-600">
                        จัดการและติดตามคำสั่งซื้อทั้งหมดของคุณ
                    </p>
                </div>
            </div>
        </div>
        ...
        {:else if error}
            <div
                class="bg-white rounded-2xl shadow-sm border border-red-200 p-8"
            >
        ...
        {:else if !currentUser?.id}
            <div
                class="bg-white 0 rounded-2xl shadow-sm border border-gray-200 p-16"
            >
        ...
        {:else if orders.length === 0}
            <div
                class="bg-white rounded-2xl shadow-sm border border-gray-200 p-16"
            >
                <div class="text-center">
                    <div
                        class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-6"
                    ...
                    <p class="text-gray-600">
                        เมื่อคุณทำการสั่งซื้อแล้ว รายการจะแสดงที่นี่
                    </p>
                </div>
            </div>
        {:else}
            <div class="space-y-6">
                {#each orders as order (order.id)}
                    <div
                        class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-200"
                    >
                        <div
                            class="p-6 bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-200"
                        >
                            <div
                            ...
                            >
                                ...
                                <div class="flex items-center gap-4">
                                    <span
                                        class="inline-flex items-center px-3 py-1.5 rounded-xl text-sm font-medium border {getStatusClasses(
                                            order.status,
                                        )}"
                                    >
                                ...
                                <div class="flex items-center gap-4 mb-4">
                                    <div class="flex -space-x-2">
                                        ...
                                        {#if order.order_items.length > 3}
                                            <div
                                                class="w-12 h-12 rounded-lg bg-gray-100 border-2 border-white flex items-center justify-center"
                                            >
                                                <span
                                    ...
                                    >
                                        {expandedOrders.has(order.id)
                                            ...
                                            : "ดูรายละเอียด"}
                                        <svg
                                            class="w-4 h-4 transform transition-transform duration-200 {expandedOrders.has(
                                                order.id,
                                            )
                                                ? 'rotate-180'
                                                : ''}"
                                            ...
                                            >
                                                ...
                                                >
                                                    <div class="relative group">
                                                        ...
                                                        />
                                                        <div
                                                            class="absolute inset-0 bg-opacity-0 group-hover:bg-opacity-20 rounded-lg transition-all duration-200 flex items-center justify-center"
                                                        >
                                                            <svg
                                                            ...
                                                            >
                                                                <path
                                                                    d="M12 9a3 3 0 000 6 3 3 0 000-6z"
                                                                />
                                                                <path
                                                                    d="M12 1l9.5 5.5v11L12 23l-9.5-6V6.5L12 1zm0 2.311L4.5 7.653v8.694l7.5 4.342 7.5-4.342V7.653L12 3.311z"
                                                                />
                                                            </svg>
                                                        </div>
                                                    </div>
