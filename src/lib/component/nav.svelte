<script>
  ...
  const unsubscribeAuth = authStore.subscribe((storeState) => {
    if (storeState && storeState.user) {
      ...
      // Load cart from Redis when user logs in
      const email = userProfile.email || userProfile.user?.email;
      if (email) {
        cartStore.loadFromRedis(email);
      }
    } else {
      userProfile = null;
    }
  ...
  onMount(() => {
    // No LocalStorage initialization needed for Redis-only cart
  });
...
<nav class="py-3 border-gray-200 border-b-2">
  <div
    class="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center my-1"
  >
    <div class="flex space-x-4">
      <a
        ...
        class="text-sm text-slate-600 hover:text-violet-600"
      >
        <svg
    ...
    <div class="flex space-x-4">
      {#if userProfile}
        <a href="/profile" class="text-xs text-slate-600 hover:text-violet-600"
          >โปรไฟล์ <b class="font-bold"
            >{userProfile.user_metadata.username || "N/A"}</b
          ></a
        >
        <button
          on:click={async () => {
          ...
          class="text-xs text-slate-500 hover:text-red-600 transition-colors ml-2"
          >ออกจากระบบ</button
        >
      {:else}
        <a href="/signup" class="text-xs text-slate-600 hover:text-violet-600"
          >สมัครสมาชิก</a
        >
        <a href="/login" class="text-xs text-slate-600 hover:text-violet-600"
          >เข้าสู่ระบบ</a
        >
      {/if}
    </div>
  ...
  >
    <div class="ms-5">
      <a href="/" class="text-2xl font-semibold text-violet-600 ubuntu-regular"
        >Valeria</a
      >
    </div>
    ...
    <div class="relative flex-grow max-w-2xl mx-4">
      ...
      {#if isDropdownVisible}
        <ul
          class="absolute z-20 w-full bg-white border border-gray-300 mt-1 rounded-md shadow-lg"
          on:click={() => handleSearch()}
        >
          <li
    ...
    <div class="flex max-w-7xl space-x-6">
      ...
      <p
        class="relative text-slate-600 hover:text-violet-600"
        on:click={GotoMyDownload}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          width="24"
  ...
  <div
    class="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-center items-center"
  >
    <p
      class="text-xs text-slate-600 mx-6 mt-2 hover:text-slate-800 cursor-pointer border-b-2 border-transparent transition delay-50 hover:border-b-violet-500"
    >
      รูปถ่าย
    </p>
    <p
